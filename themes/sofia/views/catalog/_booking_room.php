...
<div class="booking-block">
	<header>
		<h1>Забронировать квартиру - <?=CHtml::encode($room->address)?></h1>
	</header>
	<?php $form=$this->beginWidget('CActiveForm', array(
		...
		<?=$form->hiddenField($model, 'id', array('value' => $room->id))?>
		...
		<?=CHtml::hiddenField('subject', 'Заявка на бронирование квартиры - '.CHtml::encode($room->address).' № - '.CHtml::encode($room->number))?>
		<table>
			<tr class="label">
			...
			<?php echo CHtml::ajaxLink(
				...
				array(
					...
					'success' => 'js:function(data){
						...
						if(data != "ok"){
							//console.log(data);
							$(".fancybox-inner #fancy-room").html(data);
							$("#phone-room").mask("+7 (999) 999-99-99");
							jQuery("#date-room").datepicker(jQuery.extend({showMonthAfterYear:false},jQuery.datepicker.regional["ru"],{"showAnim":"fold","dateFormat":"dd.mm.yy","minDate":0}));
				...
				array('class' => 'blue-button', 'id' => 'fancy-room-button')); 
			?>
		</div>
