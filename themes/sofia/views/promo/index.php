<section class="left">
	<?php $this->renderPartial('/catalog/_filter', array('model' => $model, 'areas' => $areas));?>	
	
	<script type="text/javascript">
		defaultFilterMin = 1000;
	</script>		
</section>
<section class="right">
	<div class="top-block">
		<h2>Квартиры по 1200</h2>
		<div class="oldPrice">вместо <span>1500</span></div>
		<a id="link-share" class="gray-button" href="/favorites/"><i class="plus-gray"></i> Квартиры в закладках</a>
	</div>
	<div class="room">
		<a class="link-addFavorites active" href="#"><span></span></a>
		<div class="room-images">
				<img src="images/tmp/room.jpg">
		</div>
		<a href="#" class="link-room">1 комнатная квартира, 50 лет октября</a>
		<div class="sleepers">
			<label>4 спальных места</label>
			<i class="sleeper"></i>
			<i class="sleeper"></i>
			<i class="sleeper"></i>
			<i class="sleeper"></i>
		</div>
		<div class="dop">
			<label class="text-title">Дополнительно</label><br>
			<div class="wifi-blue">
				<div class="title">
					<div class="top"></div>
					<div class="bottom">В квартире есть Wi-Fi</div>
				</div>
			</div>
			<div class="tele-blue">
				<div class="title">
					<div class="top"></div>
					<div class="bottom">В квартире есть Кабельное TV</div>
				</div>
			</div>
			<div class="wash-blue">
				<div class="title">
					<div class="top"></div>
					<div class="bottom">В квартире есть Стиральная машинка</div>
				</div>
			</div>
			<div class="iron-blue">
				<div class="title">
					<div class="top"></div>
					<div class="bottom">В квартире есть Утюг</div>
				</div>
			</div>
		</div>
		<div class="room-price">
			<div class="col3-left">
				<b>1500</b> руб. в сутки
			</div>
			<div class="col3-left">
				<b>600</b> руб. за ночь
			</div>
			<div class="col3-left">
				<b>300</b> руб. за час
			</div>
			<div class="clear"></div>
		</div>
	</div>

	<div class="room">
		<a class="link-addFavorites" href="#"><span></span></a>
		<div class="room-images">
				<img src="images/tmp/room.jpg">
		</div>
		<a href="#" class="link-room">1 комнатная квартира, 50 лет октября</a>
		<div class="sleepers">
			<label>4 спальных места</label>
			<i class="sleeper"></i>
			<i class="sleeper"></i>
			<i class="sleeper"></i>
			<i class="sleeper"></i>
		</div>
		<div class="dop">
			<label class="text-title">Дополнительно</label><br>
			<div class="wifi-blue">
				<div class="title">
					<div class="top"></div>
					<div class="bottom">В квартире есть Wi-Fi</div>
				</div>
			</div>
			<div class="tele-blue">
				<div class="title">
					<div class="top"></div>
					<div class="bottom">В квартире есть Кабельное TV</div>
				</div>
			</div>
			<div class="wash-blue">
				<div class="title">
					<div class="top"></div>
					<div class="bottom">В квартире есть Стиральная машинка</div>
				</div>
			</div>
			<div class="iron-blue">
				<div class="title">
					<div class="top"></div>
					<div class="bottom">В квартире есть Утюг</div>
				</div>
			</div>
		</div>
		<div class="room-price">
			<div class="col3-left">
				<b>1500</b> руб. в сутки
			</div>
			<div class="col3-left">
				<b>600</b> руб. за ночь
			</div>
			<div class="col3-left">
				<b>300</b> руб. за час
			</div>
			<div class="clear"></div>
		</div>
	</div>

	<div class="room">
		<a class="link-addFavorites" href="#"><span></span></a>
		<div class="room-images">
				<img src="images/tmp/room.jpg">
		</div>
		<a href="#" class="link-room">1 комнатная квартира, 50 лет октября</a>
		<div class="sleepers">
			<label>4 спальных места</label>
			<i class="sleeper"></i>
			<i class="sleeper"></i>
			<i class="sleeper"></i>
			<i class="sleeper"></i>
		</div>
		<div class="dop">
			<label class="text-title">Дополнительно</label><br>
			<div class="wifi-blue">
				<div class="title">
					<div class="top"></div>
					<div class="bottom">В квартире есть Wi-Fi</div>
				</div>
			</div>
			<div class="tele-blue">
				<div class="title">
					<div class="top"></div>
					<div class="bottom">В квартире есть Кабельное TV</div>
				</div>
			</div>
			<div class="wash-blue">
				<div class="title">
					<div class="top"></div>
					<div class="bottom">В квартире есть Стиральная машинка</div>
				</div>
			</div>
			<div class="iron-blue">
				<div class="title">
					<div class="top"></div>
					<div class="bottom">В квартире есть Утюг</div>
				</div>
			</div>
		</div>
		<div class="room-price">
			<div class="col3-left">
				<b>1500</b> руб. в сутки
			</div>
			<div class="col3-left">
				<b>600</b> руб. за ночь
			</div>
			<div class="col3-left">
				<b>300</b> руб. за час
			</div>
			<div class="clear"></div>
		</div>
	</div>
	<div class="clear"></div>

	<section id="text-content">
		<h1>Весь июнь действует акция</h1>
		<p>Предлагаем вам в аренду квартиры на сутки и более. Каким бы ни был ваш выбор все квартиры с качественным ремонтом, меблированы, оборудованы бытовой техникой, чистые и уютные.</p>
		<p>Предлагаем вам в аренду квартиры на сутки и более. Каким бы ни был ваш выбор все квартиры с качественным ремонтом, меблированы, оборудованы бытовой техникой, чистые и уютные.</p>
	</section>

	<section id="order"><?php $this->renderPartial('/catalog/_booking_form');?></section>
</section>
<div class="clear"></div>